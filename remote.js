...
const REMOTE_TEXT_RESONANCE_URL = REMOTE_MOMENTS_URL
  ...
  : "";
/** Activar solo cuando la Edge `text-resonance` esté desplegada con CORS (evita errores de preflight en consola). */
const USE_REMOTE_TEXT_RESONANCE = false;
...
/** Tras fallo de red/CORS en text-resonance, no repetir POST en esta carga (menos ruido en consola). */
let textResonanceFetchSkipped = false;
...
async function fetchTextResonanceRemote(lastMoment, neighborMoments) {
  if (!USE_REMOTE_TEXT_RESONANCE || !isRemoteReady() || !REMOTE_TEXT_RESONANCE_URL || textResonanceFetchSkipped) {
    return null;
  }
  ...
  try {
    ...
    textResonanceFetchSkipped = true;
    return null;
  } finally {
    scoped.clear();
  }
